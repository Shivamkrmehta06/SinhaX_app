...
bt = re.sub(r'  // ── Static data ───────────────────────────────────────────────────────────.*?// ── Lifecycle ─────────────────────────────────────────────────────────────', '// ── Lifecycle ─────────────────────────────────────────────────────────────', bt, flags=re.DOTALL)
...
st = st.replace("  Widget build(BuildContext context) {", "  Widget build(BuildContext context, WidgetRef ref) {")
# Fix the ConsumerStatefulWidget build method which shouldn't have WidgetRef ref
st = st.replace("  @override\n  Widget build(BuildContext context, WidgetRef ref) {\n    return Scaffold(\n", "  @override\n  Widget build(BuildContext context) {\n    return Scaffold(\n")
# Fix _SettingsField and _BrokerCard and _PlanCard which remain StatelessWidget
st = st.replace("class _SettingsField extends ConsumerWidget {", "class _SettingsField extends StatelessWidget {")
st = st.replace("class _BrokerCard extends ConsumerWidget {", "class _BrokerCard extends StatelessWidget {")
st = st.replace("class _PlanCard extends ConsumerWidget {", "class _PlanCard extends StatelessWidget {")
# Remove WidgetRef ref from their build methods
st = st.replace("Widget build(BuildContext context, WidgetRef ref) {\n    return Column(\n      crossAxisAlignment", "Widget build(BuildContext context) {\n    return Column(\n      crossAxisAlignment")
st = st.replace("Widget build(BuildContext context, WidgetRef ref) {\n    final initial", "Widget build(BuildContext context) {\n    final initial")
st = st.replace("Widget build(BuildContext context, WidgetRef ref) {\n    final isPopular", "Widget build(BuildContext context) {\n    final isPopular")
...
st = st.replace("ref.watch(userProvider).user.name", "ref.watch(userProvider).user.name", 1) # wait, replacing multiple times? The first one is _nameCtrl

# Actually, _nameCtrl is in _SettingsScreenState.
